"""
CCD APT Analysis Pipelines - Unified Architecture

Factory functions for creating APT analysis pipelines.
All analysis now uses a single unified pipeline that handles both
single samples (n=1) and batches (n>1) identically.
"""

import logging
from typing import Callable, List, Optional

from .ccd_apt_controllers import (
    # Unified pipeline controllers
    InitialCCDAnalysisController,
...
def create_unified_apt_pipeline(
    model,
...
def create_fftnmf_pipeline(
    ...
    """
    DEPRECATED: Use create_unified_apt_pipeline instead.
    ...
    logger.warning(
        ...
        "Use create_unified_apt_pipeline() instead."
    )
    return create_unified_apt_pipeline(
        model=model,
        logger=logger,
        generation_config=generation_config,
        safety_settings=safety_settings,
...
def create_batch_only_pipeline(
    ...
    """
    DEPRECATED: Use create_unified_apt_pipeline with preset_params instead.
    """
    logger.warning(
        "create_batch_only_pipeline() is deprecated. "
        "Use create_unified_apt_pipeline(preset_params=...) instead."
    )
    return create_unified_apt_pipeline(
        model=model,
        logger=logger,
...
def create_quick_series_pipeline(
    ...
    logger.warning(
        ...
        "Use create_unified_apt_pipeline() with enable_human_feedback=False."
    )
