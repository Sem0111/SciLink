...
The CCD scripts are located in [analyze/apt-ccd/](./../../).
...
```bash
python analyze/apt-ccd/apt_preprocessing.py --data <data_file> --rrng <rrng_file> --savedir <output_dir>
```
...
script_dir = os.path.abspath("analyze/apt-ccd")
pkg_name = "apt_ccd"
...
The resulting per-sample XYZ files share the same community label space as the merged run, so communities are directly comparable across samples.